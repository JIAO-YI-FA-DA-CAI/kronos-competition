...
class BaseBars(ABC):
    ...
    def __init__(self, metric: str, batch_size: int = 2e7):
        ...
        self.cum_statistics = {'cum_ticks': 0, 'cum_dollar_value': 0, 'cum_volume': 0, 'cum_buy_volume': 0, 'cum_maker_volume': 0}
    ...
    def batch_run(self, file_path_or_df: Union[str, Iterable[str], pd.DataFrame], verbose: bool = True, to_csv: bool = False,
        ...
        count = 0
        final_bars = []
        cols = ['date_time', 'tick_num', 'open', 'high', 'low', 'close', 'volume', 'cum_buy_volume', 'cum_maker_volume', 'cum_ticks',
                'cum_dollar_value']
    ...
    def _create_bars(self, date_time: str, price: float, high_price: float, low_price: float, list_bars: list) -> None:
        ...
        cum_maker_volume = self.cum_statistics['cum_maker_volume']
        ...
        # Update bars
        list_bars.append(
            [date_time, self.tick_num, open_price, high_price, low_price, close_price, volume, cum_buy_volume, cum_maker_volume,
             cum_ticks,
             cum_dollar_value])
...
class BaseImbalanceBars(BaseBars):
    ...
    def _extract_bars(self, data: Tuple[dict, pd.DataFrame]) -> list:
        ...
        for row in data:
            ...
            if maker_flag:
                self.cum_statistics['cum_maker_volume'] += volume
